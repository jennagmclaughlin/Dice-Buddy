...
module.exports = {
    ...
    async execute(interaction) {
        ...
        let characterProfile = await Character.findOne({ characterName: characterName });

        try {
            // searching to see if the user already has a character with inputted name
            const existingCharacter = await Character.findOne({ userID: interaction.user.id, characterName: characterName });
 
            if (existingCharacter) {
                return interaction.reply(`You already have a character named "${characterName}"!`);
            } else {
